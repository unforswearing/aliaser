...
> [!IMPORTANT]
> `aliaser` is written in cyrrently `bash`, tested interactively in `zsh` on MacOS and passes most `shellcheck` tests. This script has not been tested on any Linux-based systems. Compatibility with Linux-based systems (to include the removal of MacOS-based dependencies) will be available in v3.
